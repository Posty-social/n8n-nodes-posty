...
import { ALLOWED_MEDIA_CONTENT_TYPES, MEDIA_CONTENT_TYPE_OPTIONS } from '../../shared/constants';
		...
		options: [
			{
				...
				name: 'contentType',
				type: 'options',
				options: [{ name: 'Auto-Detect From Binary Data', value: 'auto' }, ...MEDIA_CONTENT_TYPE_OPTIONS],
				default: 'auto',
			...
			},
			{
				...
				type: 'string',
				default: '',
				description: 'Name shown in the media library. Defaults to the file name.',
			},
			{
...
export async function mediaUploadPreSend(
	...
	const requestedType = this.getNodeParameter('options.contentType', 'auto') as string;
	const contentType = requestedType === 'auto' ? binaryData.mimeType : requestedType;
	...
	const title = (requestedTitle || binaryData.fileName || 'Upload').slice(0, 500);
...
export async function mediaWaitUntilReady(
	...
	const timeoutSeconds = this.getNodeParameter('options.waitTimeout', 120) as number;
	...
	for (const item of items) {
		...
		if (media.status === 'failed') {
			throw new NodeOperationError(node, `Posty could not process media ${media.id}`);
		}
