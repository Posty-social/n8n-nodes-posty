    ...
    options: [
      {
        ...
        default: "none",
        description:
          "Review process the post must pass before it can be published",
      },
      {
        ...
        default: "",
        description:
          "When the post should be published. Leave empty to create a draft.",
      },
    ],
  },
];
...
export async function postCreatePreSend(
  ...
  if (!Array.isArray(entries) || entries.length === 0) {
    throw new NodeOperationError(
      node,
      'Add at least one channel under "Content"'
    );
  }
  ...
  if (
    typeof additionalFields.approvalMode === "string" &&
    additionalFields.approvalMode !== ""
  ) {
    body.approvalMode = additionalFields.approvalMode;
  }
  const scheduledAt = toIsoString(
    additionalFields.scheduledAt,
    node,
    "Scheduled At"
  );
  if (scheduledAt) {
