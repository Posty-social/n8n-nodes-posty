...
import { mediaUploadDescription, mediaUploadPreSend, mediaWaitUntilReady } from './upload';
		...
		options: [
			{
				...
				routing: {
					request: { method: 'POST', url: '=/v1/media/{{$parameter.mediaId}}/complete' },
				},
			},
			{
			...
			},
			{
				...
				routing: {
					request: { method: 'DELETE', url: '=/v1/media/{{$parameter.mediaId}}' },
			...
			},
			{
				...
				description: 'Upload a binary file from the input item to the media library',
				routing: {
					request: { method: 'POST', url: '/v1/media' },
